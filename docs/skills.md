...
**战局模式（与技能池无关）**：主菜单「合约模式」下可选**战役**或**清剿**。战役为黑域打字机叙事（清剿不播放），单阶段目标量高于清剿；推进中会遭遇**同事复制体**，进入第 12 阶段时面对**终幕·你的复写**。完成第 12 阶段主线后可继续清剿或撤离结算。撤离信标约在开局 8 分钟后解锁，详见仓库根目录 `README.md` 中「模式与撤离」。

### 战役人设与世界观（与技能池无关）

与 [`docs/story-world.md`](story-world.md)、[`src/game/content/stageLore.ts`](../src/game/content/stageLore.ts) 局内叙事一致：

- **杂兵**：系统在战场上堆采样时烧掉的 **token**，清掉等于帮对面打穿预算。
- **普通首领（随机遭遇的 boss）**：公司**技能模型**从**真实同事**的航迹与操作数据里拷出来的**同事复制体**；炮击型 / 冲锋型只是不同权重，不是两个物种。
- **第 9 阶段后的「结点」强化 boss**：同一套「同事复制」管线被**加料重训**后的版本，块头与血池高于随机遭遇，作为阶段结点验收（局内称**强化同事复制体** / **同事副本**）。
...
- **首次遭遇**：若三选一中出现尚未领取过、未写入图鉴的升级，会显示「首次遭遇」提示与卡片高亮；领取后写入 `discoveredUpgradeIds` 即不再视为首次。
...
- 击破复制体（引擎类型仍为 boss）后会立即弹出一次高阶奖励：
  - 常规复制体击破：弹出偏 `epic` 的**副本宝箱**三选一（`boss-epic`）
  - 每累计击破 3 次复制体：触发一次 `legendary` 传说奖励三选一（`boss-legendary`）
- 结算复盘至少应包含：
  - 本轮武器与等级
  - 关键升级
  - 完成任务数
